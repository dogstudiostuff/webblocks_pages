(function () {
  ...
  class WebBlocksRenderInfo extends Blockly.zelos.RenderInfo {
    ...
    finalizeOutputConnection_() {
      ...
      var extra = 0;
      try {
        extra = Math.round(Math.max(0, this.width * 0.12));
      } catch (e) {
        extra = Math.ceil((this.outputConnection.height || 0) * 0.15);
      }
      ...
      if (this.rightSide) {
        ...
        try {
          if (this.topRow && typeof this.topRow.xPos === 'number') this.topRow.xPos += delta;
          if (this.bottomRow && typeof this.bottomRow.xPos === 'number') this.bottomRow.xPos += delta;
        ...
        if (console && console.log) {
          try {
            console.log('WB_RENDER: finalizeOutputConnection_', {
              block: this.block_ && this.block_.type,
              h: h,
              pad: pad,
              origStartX: origStartX,
              newStartX: newStartX,
              delta: delta,
              width: this.width,
              rows: (this.rows && this.rows.length) || 0
            });
          } catch (e) { }
        }
